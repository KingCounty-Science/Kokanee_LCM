...
years = 100 #how many years would we like to run the model
runs = 2 # a run is going through the cycle for the number of years desired. how many runs do we do

## Create an empty matrix for filling in a loop 
#natural fish df
nat_df <- matrix(data = NA, nrow = years, ncol = 5)
#rownames(nat_df) <- c("SPAWN_01", "SPAWN_02","SPAWN_03","SPAWN_04","SPAWN_05", "SPAWN_06", "SPAWN_07","SPAWN_08","SPAWN_09","SPAWN_10")
colnames(nat_df) <- c("2yo", "3yo","4yo","5yo", "sum_spawn")

#hatchery fish df
hat_df <- matrix(data = NA, nrow = years, ncol = 5)
#rownames(hat_df) <- c("SPAWN_01", "SPAWN_02","SPAWN_03","SPAWN_04","SPAWN_05", "SPAWN_06", "SPAWN_07","SPAWN_08","SPAWN_09","SPAWN_10")
colnames(hat_df) <- c("2yo", "3yo","4yo","5yo", "sum_spawn")
...
for(i in 1:4)
{
  ...
  nat_df[i,"sum_spawn"] <- sum(nat_df[i,1:4], na.rm = TRUE)
  ...
  hat_df[i,"sum_spawn"] <- sum(hat_df[i,1:4], na.rm = TRUE)
}
...
for(i in 5:(years-10))
{
  ...
  nat_df[i+3,"4yo"] <- n_nat_year_4_spawners
  nat_df[i+4,"5yo"] <- n_nat_year_5_spawners
  nat_df[i,"sum_spawn"] <- sum(nat_df[i,1:4])
  ...
  hat_df[i+3,"4yo"] <- n_hat_year_4_spawners
  hat_df[i+4,"5yo"] <- n_hat_year_5_spawners
  hat_df[i,"sum_spawn"] <- sum(hat_df[i,1:4])
...
#for each year, what if the first thing we did was to pull our portions and factors that will occur that year as a list at the front. This is Neala's structure.

#before running the model, we want to create a place to index/track the values we are using. 

# go through the life cycle 5-6 times to seed the model, i.e., get starting spawner totals
...
nat_df["SPAWN_03","2yo"] <- n_nat_year_2_spawners
nat_df["SPAWN_04","3yo"] <- n_nat_year_3_spawners
nat_df["SPAWN_05","4yo"] <- n_nat_year_4_spawners
nat_df["SPAWN_06","5yo"] <- n_nat_year_5_spawners
...
hat_df["SPAWN_03","2yo"] <- n_hat_year_2_spawners
hat_df["SPAWN_04","3yo"] <- n_hat_year_3_spawners
hat_df["SPAWN_05","4yo"] <- n_hat_year_4_spawners
hat_df["SPAWN_06","5yo"] <- n_hat_year_5_spawners
...
#the first thing I need to do is go through 4 loops and get to a full cohort of spawners built from 4 imaginary years of spawning.
